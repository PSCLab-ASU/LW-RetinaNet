...
def add_fpn_retinanet_outputs(model, blobs_in, dim_in, spatial_scales):
    ...
    for lvl in range(k_min, k_max + 1):
        ...
        for nconv in range(cfg.RETINANET.NUM_CONVS):
            ...
            if lvl == k_min or lvl == k_min + 1 or nconv == 0:
                if lvl == k_min:
		    if nconv <= 3:
   		    	bl_out = model.Conv(
                            bl_in,
                ...
                bl_out = model.ConvShared(
                    ...
                    stride=1,
                    pad=1,
                    weight='retnet_cls_conv_n{}_fpn{}_w'.format(nconv, k_min+1),
                    bias='retnet_cls_conv_n{}_fpn{}_b'.format(nconv, k_min+1)
                )
        ...
        if lvl == k_min:
            ...
            retnet_cls_pred = model.ConvShared(
                ...
                pad=1,
                stride=1,
                weight='retnet_cls_pred_fpn{}_w'.format(k_min+1),
                bias='retnet_cls_pred_fpn{}_b'.format(k_min+1)
            )
        if not model.train:
            if cfg.RETINANET.SOFTMAX:
            ...
            for nconv in range(cfg.RETINANET.NUM_CONVS):
                ...
                if lvl == k_min or lvl == k_min + 1 or nconv == 0:
                    ...
                    bl_out = model.ConvShared(
                    	...
                    	stride=1,
                    	pad=1,
                    	weight='retnet_bbox_conv_n{}_fpn{}_w'.format(nconv, k_min+1),
                    	bias='retnet_bbox_conv_n{}_fpn{}_b'.format(nconv, k_min+1)
                    )
                bl_in = model.Relu(bl_out, bl_out)
